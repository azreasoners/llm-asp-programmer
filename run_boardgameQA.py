...
parser.add_argument("--dataset", type=str, help = 'Dataset from {boardgameQA}.', default = 'boardgameQA')
...
def debugger(action, input_dict, prompt_cache, prompt_cache_RAG, token_usage, pos, pos_label, num_write, reload = False, num_verifiers = 1, single_update=False, RAG = ''):
    ...
    while action != 'pass' and actions_possible!=[0,0,1]:
        ...
        if action =='update':
            ...
            ## run clingo
            entailment, stdouts, stderrs, exit_codes  = check_entailment(current_modules, timeout = args.timeout, query = 'test_query', mapping = ['I', 'proved', 'disproved', 'unknown'], strong_negation=True)
...
for prob_idx, (prob_id, instance) in enumerate(data.items()):
    ...
    if gt!='':
        ...
        if last_dict['entailment'].lower() == gt.lower():
