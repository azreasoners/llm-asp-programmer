...
from core.utils import check_entailment_credulous as check_entailment
...
parser.add_argument("--dataset", type=str, help = 'Dataset from {multiNMR-credulous}.', default='multiNMR-credulous')
...
if not args.o:
    args.o = args.dataset
assert args.dataset == 'multiNMR-credulous'
...
def debugger(action, input_dict, prompt_cache, prompt_cache_RAG, token_usage, pos, pos_label, num_write, reload = False, num_verifiers = 1, single_update=False, RAG = ''):
    test_counter = 1 # current test case number if chosen
    # use llm
    ...
    while action != 'pass' and actions_possible!=[0,0,1]:
        ...
        if not RAG:
            ...
            if 'gem' not in model:
                try:
                        ...
                        output_text =response_message.content[0].text ## check
        ...
        if action=='update':
            update_counter+=1
            if test_counter==1: # no test case happend yet, force it
                pass
        ...
        if action =='update':
            ...
            entailment, stdouts, stderrs, exit_codes  = check_entailment(current_modules, timeout = args.timeout, query = 'test_query')
...
for prob_idx, (prob_id, instance) in enumerate(data.items()):
    ...
    prob_desc = prob_desc_ + ('\n' + meta_prompts_dict[args.dataset]) # add any specific instruction for the task
    ...
    if gt!='':
        ...
        evaluation_str = 'CORRECT' if current_correct == 'True' else 'WRONG'#response.output_text if 'gem' not in evaluation_model else response.text
